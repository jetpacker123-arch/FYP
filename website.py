...
def fetch_weather(lat, lon, start=None, end=None, is_forecast=False):
    ...
    # Added solar radiation to the daily parameters
    p = {"latitude": lat, "longitude": lon, "daily": ["temperature_2m_mean", "relative_humidity_2m_mean", "shortwave_radiation_sum"], "timezone": "Asia/Hong_Kong"}
    if is_forecast: 
        p["forecast_days"] = 14
    else: 
        p["start_date"], p["end_date"] = start.strftime('%Y-%m-%d'), end.strftime('%Y-%m-%d')
    try:
...
if 'lat' not in st.session_state: 
    st.session_state.lat = 22.3193
if 'lon' not in st.session_state: 
    st.session_state.lon = 114.1694
...
# --- 4. FULL DATASET ML PIPELINE (SVR with RBF Kernel using Top 3 Features) ---
if uploaded_file:
    ...
    if st.button("🚀 Generate Planning Forecast"):
        with st.status("Training SVR Model with RBF Kernel on Historical Dataset...", expanded=True) as status:
            ...
            w_df = pd.DataFrame({
                ...
                'S': raw_w['daily']['shortwave_radiation_sum']  # Solar radiation
            })
            m_df = pd.merge(df, w_df, on='Date', how='inner')
            
            # Create lag features
            m_df['L1'] = m_df['kWh'].shift(1)
            m_df['L7'] = m_df['kWh'].shift(7)
            m_df['R7'] = m_df['kWh'].rolling(window=7).mean()
            train_set = m_df.dropna()

            # Use top 3 features with highest permutation importance: Temperature, Humidity, Solar Radiation
            # Plus the lag features
            feats = ['T', 'H', 'S', 'L1', 'L7', 'R7']
            X_raw = train_set[feats]
            y = train_set['kWh']
            ...
            model = SVR(kernel='rbf', C=300, gamma=0.001, epsilon=0.01)
            ...
            # Fetch 14-day weather forecast (includes solar radiation)
            ...
            f_df = pd.DataFrame({
                ...
                'Forecasted Solar Radiation (MJ/m²)': raw_f['daily']['shortwave_radiation_sum']
            })
            
            # Recursive forecasting for 14 days
            hist = m_df.tail(14).copy()
            preds = []
            for i in range(len(f_df)):
                row = f_df.iloc[i]
                l1 = hist['kWh'].iloc[-1]
                l7 = hist['kWh'].iloc[-7] if len(hist) >= 7 else hist['kWh'].mean()
                r7 = hist['kWh'].tail(7).mean() if len(hist) >= 7 else hist['kWh'].mean()
                curr_x = np.array([[row['Forecasted Temperature (°C)'], row['Forecasted Relative Humidity (%)'], row['Forecasted Solar Radiation (MJ/m²)'], l1, l7, r7]])
                curr_x_scaled = scaler.transform(curr_x)
                p = model.predict(curr_x_scaled)[0]
                preds.append(p)
                hist = pd.concat([hist, pd.DataFrame({'Date': [row['Date']], 'kWh': [p]})]).reset_index(drop=True)
            
            f_df['Baseline_kWh'] = preds
            status.update(label="✅ SVR-RBF Model Ready!", state="complete", expanded=False)
        ...
        m1.metric("Model R² (SVR-RBF)", f"{r2:.4f}")
        ...
        # Forecasted lines
        fig.add_trace(go.Scatter(x=f_df['Date'], y=f_df['Baseline_kWh'], name="Projected Baseline (SVR-RBF)", line=dict(dash='dash', color='blue')))
        ...
        st.dataframe(f_df.style.format({
            ...
            "Forecasted Solar Radiation (MJ/m²)": "{:.1f}",
            "Baseline_kWh": "{:.2f}", 
